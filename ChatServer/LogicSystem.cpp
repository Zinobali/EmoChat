...
void LogicSystem::RegisterHandlers() {
    ...
    handlers_[MSG_IDS::MSG_CHAT_LOGIN] = [this](std::shared_ptr<CSession> session, const uint16_t& msg_id, const std::string& msg_data) {
        ThreadPool::GetInstance()->Commit(std::mem_fn(&LogicSystem::LoginHandler), this, session, msg_id, msg_data);
        ...
        // 这里的模板推断，好像会把LoginHandler函数理解为参数为this, session, msg_id, msg_data四个参数的函数，导致编译通不过。

        // 以下两种写法都可以
        //ThreadPool::GetInstance()->Commit(std::bind(&LogicSystem::LoginHandler, this, session, msg_id, msg_data));
        //ThreadPool::GetInstance()->Commit([this, session, msg_id, msg_data]() {LoginHandler(session, msg_id, msg_data); });
